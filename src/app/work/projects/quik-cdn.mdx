---
title: "Quik - custom CDN with DNS"
publishedAt: "2025-10-01"
summary: "A custom Content Delivery Network (CDN) designed to serve static assets — images, JS/CSS, and video segments — with low latency, high availability, and edge caching."
images:
...
A production-grade **Content Delivery Network (CDN)** built from the ground up to deliver static assets — images, JavaScript, CSS, and media — with **low latency**, **high availability**, and **global caching**.
...
1. **Control Plane** – Configuration, purge API, certificate manager, and monitoring  
2. **Edge Servers (POPs)** – Handle caching, TLS termination, and rate limits  
3. **Origin / Origin Shield** – Protects backend servers from load bursts  
4. **Geo-DNS Routing** – Directs users to nearest healthy POP  
5. **Observability Layer** – Metrics (Prometheus), Logs (Elastic/Loki), Dashboards (Grafana)
...
* Automated CI/CD pipeline (Git → Build → Deploy)
...
> **Built for reliability, scalability, and speed — this CDN architecture demonstrates full-stack systems design applied to distributed infrastructure.**
